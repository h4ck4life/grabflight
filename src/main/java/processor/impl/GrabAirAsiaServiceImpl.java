...
public class GrabAirAsiaServiceImpl implements GrabAirAsiaService {
    ...
    @Override
    public JSONObject getSchedulesbyMonthRange(String destFrom,
                                               String destTo,
                                               String dateFrom,
                                               String dateTo)
            throws IOException, JSONException {

        String airAsiaScrapUrlMonthly =
                String.format(ninjaProperties.get("flight.scrap.airasia"),
        ...
        for (Element tripsEls : departReturnTableEls) {
            ...
            // Get scheduleTimeRow
            Elements scheduleTimeRowEls =
                    tripsEls.select("div.low-fare-cal-day");
            ...
            if (scheduleTimeRowEls.size() > 0) {
                for (Element scheduleTimeRow : scheduleTimeRowEls) {
                    ...
                    if (dateRows.size() > 0) {
                        date = dateRows.select("[type=radio]").get(0)
                                       .attr("value");
                    } else {
                        // date = "No Flights";
                    }
                    ...
                    // Get price
                    Elements priceRows =
                            scheduleTimeRow.select("div.low-fare-cal-day-amount");
                    if (priceRows.size() > 0) {
                    ...
                    Elements linkToPurchaseRows =
                            scheduleTimeRow.select("a[href]");
                    ...
                    if (!date.equals("") && !price.equals("")) {
                        ...
                        if (!linkToPurchase.equals("")) {
                            flightSchedule.put("linkToPurchase",
                                               airAsiaBaseURL + linkToPurchase);
                        }
